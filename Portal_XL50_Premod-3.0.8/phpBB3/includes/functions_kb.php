...
* @version $Id: functions_kb.php,v 1.1.1.1 2009/05/15 05:13:43 damysterious Exp $
...
function article_link($article_id, $page_uri)
{
	global $kb_root_path, $phpEx;
	$article_link = (KB_SEO == true && !empty($page_uri)) ? append_sid("{$kb_root_path}" . $page_uri . '.html') : append_sid("{$kb_root_path}kb_show.$phpEx", 'id=' . $article_id);
	return $article_link;
}
...
*
* $data['newest_uri'] = URI of the newest article in KB
* $data['newest_time'] = post time of the newest article in KB
* $data['newest_title'] = title of the newest article in KB
* $data['newest_id'] = ID of the newest article in KB
* $data['count_article'] = articles in DB
* $data['count_cats'] = categories in DB
*
*/
function make_categorie_list($id)
{
	global $db, $template, $kb_root_path, $user, $phpEx, $kb_config;
	$has_subs = $i = $j = 0;
	$sql = 'SELECT cat_id, cat_title, parent_id, cat_mode, description, image, bbcode_uid, bbcode_bitfield, bbcode_options, left_id, right_id, display_on_index
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		else
		{
			...
			{
				$link = (KB_SEO == true) ? append_sid("{$kb_root_path}categorie-" . $row['cat_id'] . '.html') : append_sid("{$kb_root_path}kb_categorie.$phpEx", 'id=' . $row['cat_id']);
				@$cat[$row['parent_id']]['subs'] .= '<a href="' . $link . '" class="subforum read">' . $row['cat_title'] . '</a> ';
			}
			if ($row['parent_id'] == $id)
			{
				$has_subs = true;
	...
	// Article
	$data['newest_time'] = $newest_uri = $newest_time = $newest_title = $newest_id = '0';
	...
	for ($i = 0; $row = $db->sql_fetchrow($result); $i++)
	{
		...
		@$cat[$row['cat_id']]['articles']++;
	
		if ($row['post_time'] > @$newest_cat_time[$row['cat_id']])
		{
			$newest_cat_uri[$row['cat_id']]		= $row['page_uri'];
			$newest_cat_time[$row['cat_id']]	= $row['post_time'];
			$newest_cat_title[$row['cat_id']]	= $row['titel'];
			$newest_cat_id[$row['cat_id']]		= $row['article_id'];
			$newest_cat_user[$row['cat_id']]	= get_username_string('full', $row['user_id'], $row['username'], $row['user_colour']);
		}
	
		if ($row['post_time'] > $data['newest_time'])
		{
			$data['newest_uri']		= $row['page_uri'];
			$data['newest_time']	= $row['post_time'];
			$data['newest_title']	= $row['titel'];
			$data['newest_id']		= $row['article_id'];
		}
	}
	...
	for ($i = 0; isset($main_cat[$i]['cat_id']); $i++)
	{
		...
		{
			if ($cat[$j]['parent_id'] == $main_cat[$i]['cat_id'])
			{
				$folder_image = ($cat[$j]['left_id'] + 1 != $cat[$j]['right_id']) ? 'forum_read_subforum' : 'forum_read';
				$template->assign_block_vars('maincat.ucat', array(
					'U_CATEGORIE'			=> (KB_SEO == true) ? append_sid("{$kb_root_path}categorie-" . $cat[$j]['cat_id'] . '.html') : append_sid("{$kb_root_path}kb_categorie.$phpEx", 'id=' . $cat[$j]['cat_id']),
					'S_MORE_LINK'			=> true,
					'NEWEST_ARTICLE'		=> isset($newest_cat_title[$cat[$j]['cat_id']]) ? $newest_cat_title[$cat[$j]['cat_id']] : '',
					'U_NEWEST_ARTICLE'		=> isset($newest_cat_uri[$cat[$j]['cat_id']]) ? article_link($newest_cat_id[$cat[$j]['cat_id']], $newest_cat_uri[$cat[$j]['cat_id']]) : '',
					'NEWEST_TIME'			=> isset($newest_cat_title[$cat[$j]['cat_id']]) ? $user->format_date($newest_cat_time[$cat[$j]['cat_id']]) : '',
					'AUTHOR_FULL'			=> isset($newest_cat_user[$cat[$j]['cat_id']]) ? $newest_cat_user[$cat[$j]['cat_id']] : '',
					'ARTICLES'				=> isset($cat[$cat[$j]['cat_id']]['articles']) ? $cat[$cat[$j]['cat_id']]['articles'] : '0',
					'DESCRIPTION'			=> $cat[$j]['description'],
					'CAT_IMAGE'				=> $cat[$j]['image'],
					'SUB_CAT'				=> isset($cat[$cat[$j]['cat_id']]['subs']) ? $cat[$cat[$j]['cat_id']]['subs'] : '',
					'READ_MORE'				=> sprintf($user->lang['READ_MORE'], isset($cat[$cat[$j]['cat_id']]['articles']) ? $cat[$cat[$j]['cat_id']]['articles'] : '0'),
...
function make_cat_select($select_id = false, $disable = 0, $own_id = 0)
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		{
			$disabled = ' disabled="disabled" style="font-weight: bold"';
		}
...
function activate_article($article_id)
{
	...
	$sql_array = array(
		'SELECT'	=> 'a.article_id, a.activ, a.article, a.titel, a.description, a.post_time, a.cat_id, a.type_id, a.user_id, a.page_uri, a.post_id, c.post_forum, c.cat_title, c.show_edits, t.name, ut.username, ut.user_colour, ut.user_id, au.username AS author',
	...
	if($row['post_id'] == 0 && $row['post_forum'] <> 0)
	{
		...
		submit_post('post', $subject, $row['username'], $kb_config['topic_type'], $poll, $data);
...
function update_article_post($article_id)
{
	...
	if ($kb_config['update_post'] == 1)
	{
		...
		if (!empty($post_row))
		{
			...
			submit_post('edit', $subject, '', $kb_config['topic_type'], $poll, $data);
...
function replace_post_text($row, $text)
{
	...
	$url = (KB_SEO == true && !empty($row['page_uri'])) ? generate_board_url() . '/' . KB_FOLDER . '/' . $row['page_uri'] . '.html' : generate_board_url() . '/' . KB_FOLDER . "/kb_show.$phpEx?id=" . $row['article_id'];
	// Replace variables
	$search = array(
		'{TITLE}',
		'{DESCRIPTION}',
...
{
	global $auth, $cache, $user, $db, $phpbb_root_path, $phpEx;
	if ($auth->acl_get('m_del_kb'))
	{
		if (confirm_box(true))
		{
			$sql = 'SELECT *
...
function make_type_list($id = 0)
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		$type_option .= ($id == $row['type_id']) ? '<option selected value="' . $row['type_id'] . '">' . $row['name'] . '</option>' : '<option value="' . $row['type_id'] . '">' . $row['name'] . '</option>';
	}
	$db->sql_freeresult($result);
	return $type_option;
}
