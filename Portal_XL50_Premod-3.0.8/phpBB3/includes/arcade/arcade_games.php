...
*
* @package arcade
* @version $Id: arcade_games.php 790 2009-06-19 14:29:40Z JRSweets $
...
switch($mode)
{
	...
	case 'fav':
		...
		$template->assign_block_vars('navlinks', array(
			'FORUM_NAME'		=> $user->lang['ARCADE_FAV'],
			'U_VIEW_FORUM'		=> append_sid("{$phpbb_root_path}arcade.$phpEx", "mode=fav"),
		));
	...
	case 'search':
		...
		$template->assign_block_vars('navlinks', array(
			'FORUM_NAME'		=> $user->lang['ARCADE_SEARCH'],
...
$template->assign_vars(array(
	...
	'S_ARCADE_ACTION'		=> append_sid("{$phpbb_root_path}arcade.$phpEx", "$pagination_mode&amp;start=$start"),
));
	...
	if ($row = $db->sql_fetchrow($result))
	{
		do
		{
			...
			$gamesrow = array(
				...
				'U_GAME_EDIT'			=> ($auth->acl_get('a_arcade_game') && !empty($user->data['is_registered'])) ? append_sid("{$phpbb_root_path}adm/index.$phpEx", "i=arcade_games&mode=edit_games&action=edit&g={$row['game_id']}", false, $user->session_id) : '',
				'U_SCORE_EDIT'			=> ($auth->acl_get('a_arcade_scores') && $row['game_plays'] && !empty($user->data['is_registered'])) ? append_sid("{$phpbb_root_path}adm/index.$phpEx", "i=arcade_games&mode=edit_scores&action=show_scores&g={$row['game_id']}",false, $user->session_id) : '',
				...
				'GAME_IMAGE_FIRST' 		=> ($row['game_highuser'] == $user->data['user_id'] ) ? $arcade->get_image('src', 'img', '1st.gif', '') : '' ,
			);
