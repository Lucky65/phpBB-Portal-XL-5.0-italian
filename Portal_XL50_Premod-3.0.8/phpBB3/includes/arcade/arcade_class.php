...
*
* @package arcade
* @version $Id: arcade_class.php 815 2009-07-10 15:48:01Z JRSweets $
...
class arcade
{
	...
	*/
	...
	{
		global $config, $arcade_cache, $template, $phpbb_root_path, $phpEx, $user, $auth;
		...
		$template->assign_vars(array(
			...
			'S_ARCADE_REPORTS_OPEN'	=> (!empty($this->totals['reports_open'])) ? true : false,
			
			'T_SWFOBJECT_PATH'      => "{$phpbb_root_path}arcade/js",
			...
			'ARCADE_REPORTS_OPEN'	=> ($this->totals['reports_open'] > 1) ? sprintf($user->lang['ARCADE_REPORTS_OPEN'], $this->totals['reports_open']) : sprintf($user->lang['ARCADE_REPORT_OPEN'], $this->totals['reports_open']),
		));
		...
		if ($in_arcade)
		{
			$template->assign_block_vars('navlinks', array(
				...
				'U_VIEW_FORUM'		=> append_sid("{$phpbb_root_path}arcade.$phpEx")
			));
		}
	}
	...
	*/
	function set_favorites($mode, $cat_id, $game_id)
	{
		...
		if ($mode == 'addfav' &&  $user->data['is_registered'])
		{
			...
			{
				$sql = 'INSERT INTO ' . ARCADE_FAVS_TABLE . ' ' . $db->sql_build_array('INSERT', $data);
				$db->sql_query($sql);
			}
		}
	...
	*/
	function get_image($mode, $type, $image, $alt)
	{
		...
		switch (strtolower($type))
		{
			case 'cat':
				...
				$image_path = $phpbb_root_path . $this->config['cat_image_path'];
			break;

			case 'img':
				$theme_path = "{$phpbb_root_path}styles/" . $user->theme['theme_path'] . '/theme/images/arcade/';
				$image_path = $phpbb_root_path . $this->config['image_path'];
			break;
		...
		if (file_exists($theme_path . $image))
		{
	...
	*/
	function set_rating_image($votes, $sum, $avg_score)
	{
		...
		{
			$image .= '<img src="' . $this->get_image('src', 'img', 'star1.gif', '') . '" title="' . $votes . '" alt="' . $votes . '" />';
		}

		$blank_stars = ( 5 - $rating_score);
		for ($i = 1; $i <= $blank_stars; $i++)
		{
			$image .= '<img src="' . $this->get_image('src', 'img', 'star2.gif', '') . '" title="' . $votes . '" alt="' . $votes . '" />';
		}
	...
	*/
	function set_fav_image($game_data, $game_id, $cat_id)
	{
		...
		else
		{
			...
			$image = '<a href="' . append_sid("{$phpbb_root_path}arcade.$phpEx", "mode=addfav$locate&amp;g=$game_id&amp;start=$start") . '"><img src="' . $this->get_image('src', 'img', 'favorite.gif', '') . '" border="0" title="'.$user->lang['ARCADE_ADD_FAV'].'" alt="'.$user->lang['ARCADE_ADD_FAV'].'" style="padding-top: 4px;" /></a>';

			if (isset($game_data[$game_id]))
			{
				$image = '<a href="' . append_sid("{$phpbb_root_path}arcade.$phpEx", "mode=delfav$locate&amp;g=$game_id&amp;start=$start") . '"><img src="' . $this->get_image('src', 'img', 'remove_favorite.gif', '') . '" border="0" title="'.$user->lang['ARCADE_REMOVE_FAV'].'" alt="'.$user->lang['ARCADE_REMOVE_FAV'].'" style="padding-top: 4px;" /></a>';
			}
	...
	*/
	function init_points()
	{
		...
		{
			$this->points['type'] = ULTIMATE_POINTS_SYSTEM;
		...
		$this->points['show'] = ($this->points['installed'] && $this->points['enabled'] && $this->config['use_points']) ? true : false;
	...
	*/
	function get_points($user_id = false)
	{
		...
		{
			if ($this->points['type'] == SIMPLE_POINTS_SYSTEM || $this->points['type'] == ULTIMATE_POINTS_SYSTEM)
			{
	...
	function set_jackpot($mode, $data)
	{
		...
		switch ($mode)
		{
			case 'add':
				...
				if ($this->config['jackpot_maximum'] && $jackpot > $this->config['jackpot_maximum'])
				{
					$jackpot = $this->config['jackpot_maximum'];
				}
