...
*
* @package arcade
* @version $Id: arcade_score.php 809 2009-07-02 20:27:41Z JRSweets $
...
if ($error || $game_data['cat_test'])
{
	...
	if ($game_data['cat_test'])
	{
		$message = sprintf($user->lang['ARCADE_TEST_SCORE'], $score, $game_data['game_name'], '<a href="' . $game_url . '">', '</a>', '<a href="' . $cat_url . '">', '</a>');
		if ($auth->acl_get('a_') && defined('DEBUG_EXTRA'))
		{
			$message .= '<br /><br />' . $arcade->debug($game_data);
		}
	}
	else
	{
...
$sql = 'UPDATE ' . ARCADE_PLAYS_TABLE . '
	SET total_plays = total_plays + 1, total_time = total_time + ' . $game_data['total_time'] . '
	WHERE game_id = ' . $game_data['game_id'] . '
		AND user_id = ' . $user->data['user_id'];
