...
*
* @package arcade
* @version $Id: arcade_admin_class.php 803 2009-07-01 17:55:12Z JRSweets $
...
class arcade_admin extends arcade
{
	...
	*/
	function reset($mode, $user_id = false)
	{
		...
		{
			case 'arcade':				
				$db->sql_query('DELETE FROM ' . ARCADE_FAVS_TABLE);
				$db->sql_query('DELETE FROM ' . ARCADE_PLAYS_TABLE);
				$db->sql_query('DELETE FROM ' . ARCADE_RATING_TABLE);
			...
			case 'scores':
				$db->sql_query('DELETE FROM ' . ARCADE_SCORES_TABLE);
	...
	*/
	...
	function convert_install_file($file, $xml = false)
	{
		...
		else
		{
			...
			{
				$data['game_files'] = serialize(array('arcade/gamedata/' . $ibp_config['gname'] . '/'));
			}
