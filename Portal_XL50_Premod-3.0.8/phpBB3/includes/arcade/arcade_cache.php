...
*
* @package acm
* @version $Id: arcade_cache.php 804 2009-07-01 18:36:21Z JRSweets $
	...
	*/
	function obtain_arcade_cats()
	{
		if (($cats = $this->get('_arcade_cats')) === false)
		{
			...
			$sql = 'SELECT  cat_id, cat_name, cat_style, cat_status, cat_link, cat_type, cat_test, cat_download, cat_password, cat_rules, cat_rules_link, parent_id, cat_parents, left_id, right_id, cat_desc, cat_desc_options, cat_desc_uid, cat_desc_bitfield, cat_rules_options, cat_rules_uid, cat_rules_bitfield, cat_games_per_page
	...
	*/
	function obtain_arcade_totals()
	{
		if (($row = $this->get('_arcade_totals')) === false)
		{
			...
			$sql = 'SELECT COUNT(game_id) as scores
				FROM ' . ARCADE_SCORES_TABLE;
			$result = $db->sql_query($sql);
			$row['scores'] = $db->sql_fetchfield('scores');
			$db->sql_freeresult($result);
			...
			$result = $db->sql_query($sql);
			$row['reports_open'] = $db->sql_fetchfield('reports_open');
			$db->sql_freeresult($result);
