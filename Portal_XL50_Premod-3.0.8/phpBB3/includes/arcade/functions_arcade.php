...
*
* @package arcade
* @version $Id: functions_arcade.php 803 2009-07-01 17:55:12Z JRSweets $
...
{
	global $arcade, $auth, $auth_arcade, $game_fav_data, $user, $phpbb_root_path, $phpEx, $template, $_SID;
...
{
	global $table_prefix, $template, $phpbb_root_path, $phpEx, $user;
...
function generate_arcade_nav(&$cat_data, $in_game = false)
{
	...
	if (!empty($cat_data))
	{
		...
		foreach ($cat_parents as $parent_cat_id => $parent_data)
		{
			...
			$template->assign_block_vars('navlinks', array(
				...
				'U_VIEW_FORUM'		=> append_sid("{$phpbb_root_path}arcade.$phpEx", 'mode=cat&amp;c=' . $parent_cat_id))
			);
		}
	}
	...
	$template->assign_block_vars('navlinks', array(
		...
		'U_VIEW_FORUM'		=> append_sid("{$phpbb_root_path}arcade.$phpEx", 'mode=cat&amp;c=' . $cat_data['cat_id']))
	);
	...
	if ($in_game)
	{
		$template->assign_block_vars('navlinks', array(
			...
			'U_VIEW_FORUM'		=> append_sid("{$phpbb_root_path}arcade.$phpEx", 'mode=play&amp;g=' . $cat_data['game_id']),
		));
	}
