		...
		if (!$agreed || ($coppa === false && $config['coppa_enable']) || ($coppa && !$config['coppa_enable']))
		{
			...
			if ($change_lang)
			{
				...
				$s_hidden_fields = array_merge($s_hidden_fields, array(
					...
					'country_flag'				=> request_var('country_flag', ''),					
					// Country Flags Version 3.0.6
//-- mod: Automatic DST 2 ------------------------------------------//
...
//-- mod: Automatic DST 2 ------------------------------------------//
		...
		if ($submit)
		{
			$error = validate_data($data, array(
				...
				// Country Flags Version 3.0.6
				'country_flag'		=> array('string', !$config['country_flags_require'], 2, 2),// ISO 3166-1 alpha-2 (2 letters format)				
				// Country Flags Version 3.0.6
//-- mod: Automatic DST 2 ------------------------------------------//
...
//-- mod: Automatic DST 2 ------------------------------------------//
			...
			// Country Flags Version 3.0.6
			if ($config['country_flags_require'] && !$data['country_flag'])
			{
...
//-- mod: Automatic DST 2 ------------------------------------------//
					...
					'user_lang'				=> $data['lang'],
					// Country Flags Version 3.0.6
					'user_country_flag'		=> $data['country_flag'],					
					// Country Flags Version 3.0.6
					'user_type'				=> $user_type,
		...
		// Include the language file for the AJAX stuff
		...
		$s_flag_options = '<option id="" value=""' . ((!$data['country_flag']) ? ' selected="selected"' : '') . '>' . $user->lang['SELECT_FLAG'] . '</option>';
		$flag_image = $phpbb_root_path . 'images/spacer.gif';

		while ($row = $db->sql_fetchrow($result))
		{
			$selected = ($data['country_flag'] && $row['flag_code'] == $data['country_flag']) ? ' selected="selected"' : '';
			$s_flag_options .= '<option id="' . $row['flag_image'] . '" value="' . $row['flag_code'] . '"' . $selected . '>' . $row['flag_country'] . ' (' . $row['flag_code'] . ')' . '</option>';

			if ($selected)
			{
				$flag_image = $phpbb_root_path . $config['country_flags_path'] . '/' . $row['flag_image'];
			}
		}
		$db->sql_freeresult($result);		
		// Country Flags Version 3.0.6
		...
		$template->assign_vars(array(
			...
			// Include anti bot nod
			'AB_QUESTION'		=> $data['AB_Question'],			
			// Include anti bot nod
			
			// Include the AJAX stuff
			'AJAX_CHECKS_FILE'			=> 'ajaxchecks.' . $phpEx,
			'AJAX_CHECKING_IMG'			=> $user->img('icon_ajax_checking', 'AJAX_CHECKING'),
			'L_AJAX_CHECKING_USERNAME'	=> $user->lang['AJAX_CHECKING_USERNAME'],
			'L_AJAX_CHECKING_PASSWORD'	=> $user->lang['AJAX_CHECKING_PASSWORD'],
			'L_AJAX_CHECKING_EMAIL'		=> $user->lang['AJAX_CHECKING_EMAIL'],
			// Include the AJAX stuff
			...
			// Country Flags Version 3.0.6
			'FLAGS_PATH'		=> $phpbb_root_path . $config['country_flags_path'],
			'FLAG_IMAGE'		=> $flag_image,
			'S_FLAG_OPTIONS'	=> $s_flag_options,			
			'S_FLAG_REQUIRE'	=> ($config['country_flags_require']) ? true : false,			
			// Country Flags Version 3.0.6
