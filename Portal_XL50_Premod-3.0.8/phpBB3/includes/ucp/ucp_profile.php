		...
		switch ($mode)
		{
			...
			case 'profile_info':
				...
				$data = array(
					...
					'country_flag'	=> request_var('country_flag', $user->data['user_country_flag']),
					// Country Flags Version 3.0.6
				...
				$s_flag_options = '<option id="" value=""' . ((!$user->data['user_country_flag']) ? ' selected="selected"' : '') . '>' . $user->lang['SELECT_FLAG'] . '</option>';
				$flag_image = $phpbb_root_path . 'images/spacer.gif';

				while ($row = $db->sql_fetchrow($result))
				{
					$selected = ($user->data['user_country_flag'] && $row['flag_code'] == $user->data['user_country_flag']) ? ' selected="selected"' : '';
