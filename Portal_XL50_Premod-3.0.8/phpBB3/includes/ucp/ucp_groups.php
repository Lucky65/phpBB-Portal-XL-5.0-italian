					...
					case 'edit':
						...
						if ($update)
						{
							...
							$submit_ary = array(
								...
								'country_flag'	=> request_var('group_country_flag', ''),
								// Country Flags Version 3.0.6
						...
						else
						{
							...
							// Country Flags Version 3.0.6
							$group_flag = $group_row['group_country_flag'];							
							// Country Flags Version 3.0.6
						...
						$flag_options = '<option id="" value=""' . ((!$group_flag) ? ' selected="selected"' : '') . '>' . $user->lang['USER_DEFAULT'] . '</option>';
						$flag_image = $phpbb_root_path . 'images/spacer.gif';

						while ($row = $db->sql_fetchrow($result))
						{
							$selected = ($group_flag && $row['flag_code'] == $group_flag) ? ' selected="selected"' : '';
							$flag_options .= '<option id="' . $row['flag_image'] . '" value="' . $row['flag_code'] . '"' . $selected . '>' . $row['flag_country'] . ' (' . $row['flag_code'] . ')' . '</option>';
