...
*
* @package phpBB3
* @version $Id: prime_trash_bin_2.php,v 1.0.8 2009/06/10 14:03:00 PST primehalo Exp $
* @copyright (c) 2007-2008 Ken Innes IV
...
*/
if (!defined('INCLUDES_PRIME_TRASH_BIN_2'))
{
	define('INCLUDES_PRIME_TRASH_BIN_2', true);
	...
	*/
	function update_stifled_from($old_forum_id, $new_forum_id)
	{
		...
		$sql = 'UPDATE ' . TOPICS_TABLE . " SET topic_deleted_from = $new_forum_id WHERE topic_deleted_from = $old_forum_id";
		$db->sql_query($sql);

		$sql = 'UPDATE ' . POSTS_TABLE . " SET post_deleted_from = $new_forum_id WHERE post_deleted_from = $old_forum_id";
		$db->sql_query($sql);
	}
	...
	*/
	function unstifle_update_post_counts($data)
	{
		if (!empty($data))
		{
			...
			{
				$sql = 'UPDATE ' . USERS_TABLE
						. ' SET user_posts = user_posts + ' . $count
						. ' WHERE user_id=' . $poster_id;
				$db->sql_query($sql);
			}
		}
	...
	*/
	function mcp_stifle_topic($topic_ids, $delete_post = false)
	{
		...
		{
			$sql = 'SELECT topic_status FROM ' . TOPICS_TABLE . ' WHERE topic_id=' . $topic_ids[0];
			$result = $db->sql_query($sql);
...
//trigger_error(str_replace("\n", '<br />', str_replace('<', '&lt;', print_r($sync_sql, true) . "\n\n" . $tmp)));
