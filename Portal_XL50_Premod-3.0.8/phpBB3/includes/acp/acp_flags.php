	...
	function main($id, $mode)
	{
		...
		switch ($action)
		{
			case 'save':
				...
				if ($flag_id)
				{
					$sql = 'UPDATE ' . COUNTRY_FLAGS_TABLE . ' SET ' . $db->sql_build_array('UPDATE', $sql_ary) . " WHERE flag_id = $flag_id";
				...
				{
					$sql = 'INSERT INTO ' . COUNTRY_FLAGS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
			...
			case 'delete':
				...
				if (confirm_box(true))
				{
					...
					$sql = 'DELETE FROM ' . COUNTRY_FLAGS_TABLE . "
						WHERE flag_id = $flag_id";
					$db->sql_query($sql);
			...
			case 'add':
				...
				$imglist = filelist($phpbb_root_path . $config['country_flags_path'], '');
				...
				$template->assign_vars(array(
					...
					'FLAG_IMAGE'		=> ($edit_img) ? $phpbb_root_path . $config['country_flags_path'] . '/' . $edit_img : $phpbb_admin_path . 'images/spacer.gif',
				));
