...
*
* @package acp
* @version $Id: acp_arcade_permissions.php 795 2009-06-25 18:44:31Z JRSweets $
...
class acp_arcade_permissions
{
	...
	{
		global $db, $user, $auth, $auth_arcade, $template, $table_prefix, $cache;
	...
	*/
	function set_all_permissions($mode, $permission_type, &$auth_admin, &$user_id, &$group_id)
	{
		...
		if ($mode == 'setting_category_local')
		{
			trigger_error($user->lang['AUTH_UPDATED'] . adm_back_link($this->u_action . '&amp;cat_id[]=' . implode('&amp;cat_id[]=', $cat_ids)));
		}
		else
		{
			trigger_error($user->lang['AUTH_UPDATED'] . adm_back_link($this->u_action));
	...
	*/
	function remove_permissions($mode, $permission_type, &$auth_admin, &$user_id, &$group_id, &$cat_id)
	{
		...
		if ($mode == 'setting_category_local')
		{
			trigger_error($user->lang['AUTH_UPDATED'] . adm_back_link($this->u_action . '&amp;cat_id[]=' . implode('&amp;cat_id[]=', $cat_id)));
		}
		else
		{
			trigger_error($user->lang['AUTH_UPDATED'] . adm_back_link($this->u_action));
	...
	*/
	function permission_trace($user_id, $cat_id, $permission)
	{
		...
		// Once the arcade requires 3.0.6 we will use this...
