...
*
* @package acp
* @version $Id: acp_arcade_manage.php 803 2009-07-01 17:55:12Z JRSweets $
...
class acp_arcade_manage
{
	...
	{
		global $db, $user, $auth, $auth_arcade, $template, $table_prefix, $cache, $arcade;
			...
			switch ($action)
			{
				...
				case 'g_delete':
					...
					if (confirm_box(true))
					{
						...
						trigger_error(sprintf($user->lang['GAME_DELETED'], $game_data['game_name']) . adm_back_link($this->u_action . '&amp;parent_id=' . $this->parent_id . (($start) ? "&start=$start" : '')));
					}
				...
				case 'add':
					...
					$cat_data += array(
						...
						'cat_test'				=> request_var('cat_test', false),
		...
		if ($total_games && $this->parent_id)
		{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				do
				{
					$url = $this->u_action . '&amp;parent_id=' . $this->parent_id . '&amp;g=' . $row['game_id'] . (($start) ? "&start=$start" : '');
		...
		$template->assign_vars(array(
			...
			'U_ACTION'				=> $this->u_action . '&amp;parent_id=' . $this->parent_id . (($start) ? "&start=$start" : ''),
			'U_ACTION_DELETE'		=> $this->u_action . '&amp;parent_id=' . $this->parent_id . '&amp;action=g_delete' . (($start) ? "&start=$start" : ''),
