...
*
* @package acp
* @version $Id: acp_kb.php,v 1.1.1.1 2009/05/15 05:14:12 damysterious Exp $
	...
	function delete_cat($id)
	{
		...
		if (confirm_box(true))
		{
			...
			$sql = 'UPDATE ' . KB_CATEGORIE_TABLE . "
				...
				WHERE right_id > " . $cat_data['left_id'];
			$db->sql_query($sql);
	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
				...
				switch ($action)
				{
					...
					case 'update':
						...
						$sql_ary = array(
							...
							'image'				=> $db->sql_escape(request_var('image', '')),
					...
					// Edit categorie
					case 'edit':
						$sql = 'SELECT cat_title, parent_id, description, image, show_edits, display_on_index, post_forum, cat_mode, bbcode_uid, bbcode_options
							FROM ' . KB_CATEGORIE_TABLE . '
							WHERE cat_id = ' . $id;
						$result = $db->sql_query($sql);
						$row = $db->sql_fetchrow($result);
						...
						$template->assign_vars(array(
							...
							'SHOW_EDITS0'				=> ($row['show_edits'] == '0') ? ' checked' : '',
							'SHOW_EDITS1'				=> ($row['show_edits'] == '1') ? ' checked' : '',
							'DISPLAY_ON_INDEX0'			=> ($row['display_on_index'] == '0') ? ' checked' : '',
							'DISPLAY_ON_INDEX1'			=> ($row['display_on_index'] == '1') ? ' checked' : '',
						));
					break;
			...
			case 'config':
				...
				for($i = 1; $i <= 20; $i++)
				{
					$index_topics .= ($i == $kb_config['index_topics']) ? '<option selected value="' . $kb_config['index_topics'] . '">' . $kb_config['index_topics'] . '</option>' : '<option value="' . $i . '">' . $i . '</option>';
				}

				$sort_order = ($kb_config['sort_order'] == 'titel') ?  '<option value="titel" selected>' . $user->lang['ARTICLE_TITLE'] . '</option>' :  '<option value="titel">' . $user->lang['ARTICLE_TITLE'] . '</option>';
				$sort_order .= ($kb_config['sort_order'] == 'user_id') ?  '<option value="user_id" selected>' . $user->lang['USERNAME'] . '</option>' :  '<option value="user_id">' . $user->lang['USERNAME'] . '</option>';
				$sort_order .= ($kb_config['sort_order'] == 'type_id') ?  '<option value="type_id" selected>' . $user->lang['TYPE'] . '</option>' :  '<option value="type_id">' . $user->lang['TYPE'] . '</option>';
				$sort_order .= ($kb_config['sort_order'] == 'post_time') ?  '<option value="post_time" selected>' . $user->lang['ARTICLE_POSTET'] . '</option>' :  '<option value="post_time">' . $user->lang['ARTICLE_POSTET'] . '</option>';


				$sort_order .= ($kb_config['sort_order'] == 'hits') ?  '<option value="hits" selected>' . $user->lang['VIEWED'] . '</option>' :  '<option value="hits">' . $user->lang['VIEWED'] . '</option>';




				$template->assign_vars(array(
					'S_POST_TOPIC_GLOBAL'		=> ($kb_config['topic_type'] == 3) ? ' checked="checked"' : '',
					'S_POST_TOPIC_ANNOUNCEMENT'	=> ($kb_config['topic_type'] == 2) ? ' checked="checked"' : '',
					'S_POST_TOPIC_STICKY'		=> ($kb_config['topic_type'] == 1) ? ' checked="checked"' : '',
					'S_POST_TOPIC_NORMAL'		=> ($kb_config['topic_type'] == 0) ? ' checked="checked"' : '',
					'KB_MODE0'					=> ($kb_config['kb_mode'] == '0') ? 'checked' : '',
					'KB_MODE1'					=> ($kb_config['kb_mode'] == '1') ? 'checked' : '',
					'ACTIV_TYPES0'				=> ($kb_config['activ_types'] == '0') ? 'checked' : '',
					'ACTIV_TYPES1'				=> ($kb_config['activ_types'] == '1') ? 'checked' : '',
					'KB_TITLE'					=> $kb_config['kb_title'],
					'KB_DESCRIPTION'			=> $kb_config['kb_description'],
					'POST_SUBJECT'				=> $kb_config['post_subject'],
					'POST_MESSAGE'				=> $kb_config['post_message'],
					'POST_USER'					=> $kb_config['post_user'],
					'SHOW_POST_EDIT0'			=> ($kb_config['show_post_edit'] == '0') ? 'checked' : '',
					'SHOW_POST_EDIT1'			=> ($kb_config['show_post_edit'] == '1') ? 'checked' : '',
					'CACHE_TIME'				=> $kb_config['cache_time'],
					'UPDATE_POST0'				=> ($kb_config['update_post'] == '0') ? 'checked' : '',
					'UPDATE_POST1'				=> ($kb_config['update_post'] == '1') ? 'checked' : '',
					'INDEX_TOPICS'				=> $index_topics,
					'SORT_ORDER'				=> $sort_order,
					'ORDER_ASC'					=> ($kb_config['sort_order_dir'] != 'DESC') ? 'checked' : '',
					'ORDER_DESC'				=> ($kb_config['sort_order_dir'] == 'DESC') ? 'checked' : '',
