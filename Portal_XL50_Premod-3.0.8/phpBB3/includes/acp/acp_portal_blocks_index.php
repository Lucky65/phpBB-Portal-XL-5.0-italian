...
* @version $Id: acp_portal_blocks_index.php,v 1.2 2009/10/17 damysterious Exp $
*
...
class acp_portal_blocks_index
{
    ...
    function main($id, $im_portal_block)
    {
        ...
        $this->tpl_name = 'portal_xl/acp_portal_blocks_index';
        $this->page_title = 'ACP_MANAGE_INDEX_BLOCKS';
        ...
        $table = PORTAL_BLOCK_INDEX_TABLE;		
        ...
        case 'save':
                ...
                if ($block_id)
                {               
                $sql = 'UPDATE ' . PORTAL_BLOCK_INDEX_TABLE . ' SET ' . $db->sql_build_array('UPDATE', $sql_ary) . " WHERE block_id = $block_id";

                    $message = $user->lang['BLOCK_UPDATED'];
                }
                else
                {
                     $sql = 'INSERT INTO ' . PORTAL_BLOCK_INDEX_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
            ...
            case 'delete':
				...
				if (confirm_box(true))
				{
					...
					$sql = 'DELETE FROM ' . PORTAL_BLOCK_INDEX_TABLE . "
						WHERE block_id = $block_id";
					$db->sql_query($sql);
			...
			{
				$config_left_width 		= $portal_config['portal_index_left_collumn_width'];
				$config_right_width 	= $portal_config['portal_index_right_collumn_width'];		
				$config_left_active 	= $portal_config['portal_index_left_column'];
				$config_right_active 	= $portal_config['portal_index_right_column'];
				$config_index_layout 	= $portal_config['portal_index_layout'];
			}
			...
			$template->assign_vars(array(
				...
				'LEFT_WIDTH'			=> (isset($portal_config['portal_index_left_collumn_width'])) ? $portal_config['portal_index_left_collumn_width'] : '',					
				'RIGHT_WIDTH'			=> (isset($portal_config['portal_index_right_collumn_width'])) ? $portal_config['portal_index_right_collumn_width'] : '',
				'LEFT_COLLUMN_ACTIVE'	=> (isset($portal_config['portal_index_left_column'])) ? $portal_config['portal_index_left_column'] : '',					
				'RIGHT_COLLUMN_ACTIVE'	=> (isset($portal_config['portal_index_right_column'])) ? $portal_config['portal_index_right_column'] : '',
				'INDEX_LAYOUT_ACTIVE'	=> (isset($portal_config['portal_index_layout'])) ? $portal_config['portal_index_layout'] : ''
				));					
			...
			case 'savecolumn':
				...
				$config_left_width 		= request_var('portal_index_left_collumn_width','', true);
				$config_right_width 	= request_var('portal_index_right_collumn_width','', true);
				$config_left_active 	= request_var('portal_index_left_column','', true);
				$config_right_active 	= request_var('portal_index_right_column','', true);
				$config_index_layout 	= request_var('portal_index_layout','', true);
							
 				if ($action == 'savecolumn')
				{
					$db->sql_query('UPDATE ' . PORTAL_CONFIG_TABLE . ' SET config_value = ' . $config_left_width . ' WHERE config_name = "portal_index_left_collumn_width"');
					$db->sql_query('UPDATE ' . PORTAL_CONFIG_TABLE . ' SET config_value = ' . $config_right_width . ' WHERE config_name = "portal_index_right_collumn_width"');
					$db->sql_query('UPDATE ' . PORTAL_CONFIG_TABLE . ' SET config_value = ' . $config_left_active . ' WHERE config_name = "portal_index_left_column"');
					$db->sql_query('UPDATE ' . PORTAL_CONFIG_TABLE . ' SET config_value = ' . $config_right_active . ' WHERE config_name = "portal_index_right_column"');
					$db->sql_query('UPDATE ' . PORTAL_CONFIG_TABLE . ' SET config_value = ' . $config_index_layout . ' WHERE config_name = "portal_index_layout"');
		...
		$nb_portal_block = 0 ;								
			...
			$template->assign_block_vars('portal_index_column', array(
				'LEFT_WIDTH'			=> 	$portal_config['portal_index_left_collumn_width'],
				'RIGHT_WIDTH'			=> 	$portal_config['portal_index_right_collumn_width'],
				'LEFT_COLLUMN_ACTIVE'	=> 	$portal_config['portal_index_left_column'],
				'RIGHT_COLLUMN_ACTIVE'	=> 	$portal_config['portal_index_right_column'],
				'INDEX_LAYOUT_ACTIVE'	=>  $portal_config['portal_index_layout'],
