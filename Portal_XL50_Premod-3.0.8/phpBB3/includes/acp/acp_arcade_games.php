...
*
* @package acp
* @version $Id: acp_arcade_games.php 811 2009-07-07 15:35:19Z JRSweets $
	...
	function add_games()
	{
		...
		if ($files = scandir($dir))
		{
			foreach ($files as $file)
			{
				...
				$install_xml = $dir . $file . '/game.xml';
				if (file_exists($install_file))
				{
					// Convert ibp install file
					$arcade->convert_install_file($install_file);
				}
				else if (file_exists($install_xml))
				{
					// Convert xml install file
					$arcade->convert_install_file($install_xml, true);
				}
	...
	function unpack_games()
	{
		...
		if ($update)
		{
			...
			foreach($game_array as $key => $game)
			{
				...
				{
					if (file_exists($phpbb_root_path . $tmp_path . 'game.xml'))
					{
						$is_xml = true;
						$game_config = $phpbb_root_path . $tmp_path . 'game.xml';
					}
					else
					{
						// Let check if this is an older game (before 1.0.RC5)
						if (!file_exists($phpbb_root_path . $tmp_path . 'includes/arcade/games/' . $game_name . '/' . $game_name . '.' . $phpEx))
						{
							$bad_game[] = $game;
				...
				$game_data_path = ($is_old) ? $tmp_path . 'arcade/' : $tmp_path;
				$game_file_path = ($is_old) ? $tmp_path . 'includes/arcade/games/' . $game_name . '/' : $tmp_path;

				// If the gamedata folder exists move it to the correct location
				if (file_exists($phpbb_root_path . $game_data_path . 'gamedata'))
				{
					//Copy and delete
					$file_functions->copy_dir($phpbb_root_path . $game_data_path . 'gamedata', $phpbb_root_path . 'arcade/gamedata');
					$file_functions->delete_dir($phpbb_root_path . $game_data_path . 'gamedata');
				}

				// Convert the ibp or xml install file
				$game_type = $arcade->convert_install_file($game_config, $is_xml);

				$file_functions->copy_dir($phpbb_root_path . $game_file_path, $phpbb_root_path . $arcade->config['game_path'] . $game_name);
				$file_functions->delete_dir($phpbb_root_path . $tmp_path);
				...
				if ($is_ibpro || $is_xml)
				{
					...
					{
						case AR_GAME:
							add_log('admin', 'LOG_ARCADE_UNPACK_AR_GAME', $game);
						break;
						...
						default:
							add_log('admin', 'LOG_ARCADE_UNPACK_NOSCORE_GAME', $game);
						break;
					}
				}
				else
	...
	function edit_scores()
	{
		...
		switch ($action)
		{
			...
			case 'edit':
				...
				$sql_array = array(
					'SELECT'	=> 'g.*, s.*, p.total_time, p.total_plays, u.username, u.user_colour',
			...
			case 'show_scores':
				...
				$sql_array = array(
					'SELECT'	=> 's.*, p.total_time, p.total_plays, u.user_id, u.user_colour, u.username',
		...
		$tables = array(ARCADE_FAVS_TABLE, ARCADE_PLAYS_TABLE, ARCADE_RATING_TABLE, ARCADE_SCORES_TABLE);
