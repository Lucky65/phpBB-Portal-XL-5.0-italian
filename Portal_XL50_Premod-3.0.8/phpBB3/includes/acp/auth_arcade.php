...
*
* @package phpBB3
* @version $Id: auth_arcade.php 764 2009-03-27 18:04:28Z JRSweets $
...
* @package phpBB3
*/
class auth_arcade_admin extends auth_arcade
{
	/**
	* Init auth settings
	*/
	...
	*/
	function get_mask($mode, $user_id = false, $group_id = false, $cat_id = false, $auth_option = false, $scope = false, $acl_fill = ACL_NEVER)
	{
		...
		if (!$view_user_mask)
		{
			if ($cat_id !== false)
			{
				$hold_ary = ($group_id !== false) ? $this->acl_group_raw_data($group_id, $auth_option . '%', $cat_id) : $this->$acl_user_function($user_id, $auth_option . '%', $cat_id);
			}
			else
			{
		...
		// Make sure hold_ary is filled with every setting (prevents missing categorys/users/groups)
		$ug_id = ($group_id !== false) ? ((!is_array($group_id)) ? array($group_id) : $group_id) : ((!is_array($user_id)) ? array($user_id) : $user_id);
		$cat_ids = ($cat_id !== false) ? ((!is_array($cat_id)) ? array($cat_id) : $cat_id) : (($scope == 'global') ? array(0) : array());
		...
		// If cat_ids is false and the scope is local we actually want to have all categorys within the array
		if ($scope == 'local' && !sizeof($cat_ids))
		{
			$sql = 'SELECT cat_id
		...
		if ($view_user_mask)
		{
			...
			while ($userdata = $db->sql_fetchrow($result))
			{
				...
				{
					$hold_ary[$userdata['user_id']][$c_id] = array();
					foreach ($compare_options as $option)
					{
						$hold_ary[$userdata['user_id']][$c_id][$option] = $auth2->acl_get($option, $c_id);
					}
				}
			}
			$db->sql_freeresult($result);
		...
		foreach ($ug_id as $_id)
		{
			...
			foreach ($cat_ids as $c_id)
			{
				if (!isset($hold_ary[$_id][$c_id]))
				{
					$hold_ary[$_id][$c_id] = array();
				}
			}
		}
		...
		{
			$hold_ary[($group_id !== false) ? $group_id : $user_id][(int) $cat_id] = $compare_options;
		}
	...
	*/
	function get_role_mask($role_id)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			$hold_ary[$row['cat_id']]['users'][] = $row['user_id'];
		}
		$db->sql_freeresult($result);
		...
		while ($row = $db->sql_fetchrow($result))
		{
			$hold_ary[$row['cat_id']]['groups'][] = $row['group_id'];
		}
		$db->sql_freeresult($result);
	...
	*/
	function display_mask($mode, $permission_type, &$hold_ary, $user_mode = 'user', $local = false, $group_display = true)
	{
		...
		if ($local)
		{
			$cat_names_ary = arcade_admin::make_cat_select(false, false, true, false, false, false, true);

			// Remove the disabled ones, since we do not create an option field here...
			foreach ($cat_names_ary as $key => $value)
			{
				if (!$value['disabled'])
				{
					continue;
				}
				unset($cat_names_ary[$key]);
			}
		}
		else
		{
			$cat_names_ary[0] = $l_acl_type;
		}
		...
		// If we only have one category id to display or being in local mode and more than one user/group to display,
		// we switch the complete interface to group by user/usergroup instead of grouping by category
		// To achieve this, we need to switch the array a bit
		if (sizeof($cat_ids) == 1 || ($local && sizeof($ug_names_ary) > 1))
		{
			$hold_ary_temp = $hold_ary;
			$hold_ary = array();
			foreach ($hold_ary_temp as $ug_id => $row)
			{
				foreach ($cat_names_ary as $cat_id => $cat_row)
				{
					if (isset($row[$cat_id]))
					{
						$hold_ary[$cat_id][$ug_id] = $row[$cat_id];
					}
				}
			}
			unset($hold_ary_temp);

			foreach ($hold_ary as $cat_id => $cat_array)
			{
				$content_array = $categories = array();
				$this->build_permission_array($hold_ary[$cat_id], $content_array, $categories, array_keys($ug_names_ary));
		...
		else
		{
			foreach ($ug_names_ary as $ug_id => $ug_name)
			{
				...
				$content_array = $categories = array();
				$this->build_permission_array($hold_ary[$ug_id], $content_array, $categories, array_keys($cat_names_ary));
	...
	*/
	function acl_add_option($options)
	{
		...
		{
			$db->sql_multi_insert(ACL_ARCADE_OPTIONS_TABLE, $add_options);
		}
	...
	*/
	function acl_set($ug_type, $cat_id, $ug_id, $auth, $role_id = 0, $clear_prefetch = true)
	{
		global $db;

		// One or more categorys
		if (!is_array($cat_id))
		{
			$cat_id = array($cat_id);
		}
	...
	*/
	function acl_delete($mode, $ug_id = false, $cat_id = false, $permission_type = false)
	{
		global $db;

		if ($ug_id === false && $cat_id === false)
		{
			return;
		}
		...
		if ($cat_id !== false)
		{
			$where_sql[] = (!is_array($cat_id)) ? 'cat_id = ' . (int) $cat_id : $db->sql_in_set('cat_id', array_map('intval', $cat_id));
		}
		...
		if ($permission_type !== false)
		{
			...
			while ($row = $db->sql_fetchrow($result))
			{
				$cur_role_auth[$row['auth_role_id']][$row['cat_id']][] = $row[$id_field];
			}
			$db->sql_freeresult($result);
			...
			if (sizeof($cur_role_auth))
			{
				...
				{
					foreach ($auth_row as $c_id => $ug_row)
					{
						$this->acl_set($mode, $c_id, $ug_row, $auth_settings[$role_id], 0, false);
					}
				}
			}
		}
	...
	*/
	function assign_cat_array(&$category_array, $tpl_cat, $tpl_mask, $ug_id, $cat_id, $show_trace = false, $s_view)
	{
...
*/
	...
	*/
	function build_permission_array(&$permission_row, &$content_array, &$categories, $key_sort_array)
	{
		...
		foreach ($key_sort_array as $cat_id)
		{
			if (!isset($permission_row[$cat_id]))
			{
				continue;
			}
