...
//-- mod: Automatic DST 2 ------------------------------------------//
			...
			case 'country_flag':
				...
				if ($submit)
				{
					$flag_code = request_var('user_country_flag', '');

					$sql = 'UPDATE ' . USERS_TABLE . "
						SET user_country_flag = '$flag_code'
						WHERE user_id = $user_id";
					$db->sql_query($sql);

					trigger_error($user->lang['USER_FLAG_UPDATED'] . adm_back_link($this->u_action . '&amp;u=' . $user_id));
				}
				...
				$s_flag_options = '<option id="" value=""' . ((!$user_row['user_country_flag']) ? ' selected="selected"' : '') . '>' . $user->lang['NO_FLAG'] . '</option>';
				$flag_image = $phpbb_root_path . 'images/spacer.gif';

				while ($row = $db->sql_fetchrow($result))
				{
					$selected = ($user_row['user_country_flag'] && $row['flag_code'] == $user_row['user_country_flag']) ? ' selected="selected"' : '';
