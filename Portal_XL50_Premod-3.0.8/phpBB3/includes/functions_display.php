...
function display_forums($root_data = '', $display_moderators = true, $return_moderators = false)
{
	...
	foreach ($forum_rows as $row)
	{
		...
		// user info popup
		$last_poster_full = '<a href="' . $phpbb_root_path . 'memberlist.php?mode=viewprofile&amp;u=' . $row['forum_last_poster_id'] . '"><b style="color:#' . $row['forum_last_poster_colour'] . '" onmouseover="show_popup(' .$row['forum_last_poster_id']. ')" onmouseout="close_popup()">' . $row['forum_last_poster_name'] . '</b></a>';
...
function generate_forum_nav(&$forum_data)
{
	...
	if (!empty($forum_data['topic_title']))
	{
        $template->assign_block_vars('navlinks', array(
		'FORUM_NAME' => ($forum_data['topic_title']),
		'U_VIEW_FORUM'	=> append_sid("{$phpbb_root_path}viewtopic.$phpEx", 't=' . $forum_data['topic_id'])
		));
	}
...
function get_user_avatar($avatar, $avatar_type, $avatar_width, $avatar_height, $alt = 'USER_AVATAR', $ignore_config = false)
{
	...
	if (empty($avatar) || !$avatar_type || (!$config['allow_avatar'] && !$ignore_config))
	{
		...
		// return '<img src="images/avatars/no_avatar.png" width="' . $avatar_width . '" alt="' . ((!empty($user->lang[$alt])) ? $user->lang[$alt] : $alt) . '" />';
		// return '';
...
function get_user_flag($user_flag, &$flag_country, &$flag_code, &$flag_img, &$flag_img_src)
{
	...
	if (!empty($user_flag))
	{
		...
		$flag_img = (!empty($flags[$user_flag]['image'])) ? '<img src="' . $config['country_flags_path'] . '/' . $flags[$user_flag]['image'] . '" alt="' . $flag_text . '" title="' . $flag_text . '" />' : '';
		$flag_img_src = (!empty($flags[$user_flag]['image'])) ? $config['country_flags_path'] . '/' . $flags[$user_flag]['image'] : '';
	}
}
