...
* @file				functions_dl_fields.php 7 2011/02/13 OXPUS
* @copyright		(c) 2006 oxpus (Karsten Ude) <webmaster@oxpus.de> http://www.oxpus.de
* @copyright mod	(c) hotschi / demolition fabi / oxpus
* @license			http://opensource.org/licenses/gpl-license.php GNU Public License 
* @based on			Module functions_profile_fields.php 10324 from phpBB 3.0.7-PL1
*
*/
...
class custom_profile
{
	...
	*/
	function generate_profile_fields($lang_id)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$template->assign_block_vars('download_fields', array(
	...
	*/
	function update_profile_field_data($df_id, &$cp_data)
	{
		...
		$sql = 'UPDATE ' . DL_FIELDS_DATA_TABLE . '
		...
		if (!$db->sql_affectedrows())
		{
			...
			$sql = 'INSERT INTO ' . DL_FIELDS_DATA_TABLE . ' ' . $db->sql_build_array('INSERT', $cp_data_sql);
			$db->sql_query($sql);
	...
	*/
	function get_var($field_validation, &$profile_row, $default_value, $preview)
	{
		...
		$req_field_ident = request_var($profile_row['field_ident'], (is_numeric($default_value) ? 0 : ''));
		...
		else if ($profile_row['field_type'] == FIELD_INT)
		{
			if ($req_field_ident)
			{
				$value = request_var($profile_row['field_ident'], $default_value);
			}
			else
			{
		...
		{
			$value = ($req_field_ident) ? request_var($profile_row['field_ident'], $default_value, true) : ((!isset($this->profile_fields[$user_ident]) || $preview) ? $default_value : $this->profile_fields[$user_ident]);
	...
	*/
	function generate_date($profile_row, $preview = false)
	{
		...
		$req_fd = request_var($profile_row['field_ident'] . '_day', 0);
	...
	function get_profile_field($profile_row)
	{
		...
		switch ($profile_row['field_type'])
		{
			case FIELD_DATE:
				...
				$req_fd = request_var($var_name . '_day', 0);
			...
			case FIELD_BOOL:
				...
				{
					$var = (request_var($var_name, 0)) ? 1 : 2;
				}
				else
				{
			...
			case FIELD_INT:
				$req_fl = request_var($var_name, '', true);
				if ($req_fl === '')
				{
					$var = NULL;
				}
...
class custom_profile_admin extends custom_profile
{
	...
	*/
	...
	{
		global $user, $config, $lang_defs, $field_id;
