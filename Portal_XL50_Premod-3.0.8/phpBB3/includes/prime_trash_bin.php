...
*
* @package phpBB3
* @version $Id: prime_trash_bin.php,v 1.0.8 2009/06/10 14:03:00 PST primehalo Exp $
...
* Include only once.
*/
if (!defined('INCLUDES_PRIME_TRASH_BIN_PHP'))
{
	define('INCLUDES_PRIME_TRASH_BIN_PHP', true);
	...
	*/
	function get_stifled_topic_msg(&$topic_data)
	{
		...
		$sql = 'SELECT DISTINCT u.username, u.user_colour, f.forum_name'
				...
				. ' WHERE user_id=' . $topic_data['topic_deleted_user']
	...
	function get_stifled_post_msg(&$post_data, $message = '', $title = '')
	{
		...
		if (!isset($del_info['username']) || !isset($del_info['topic_title']))
		{
			$sql = 'SELECT DISTINCT u.username, u.user_colour, t.topic_title'
					...
					. ' WHERE u.user_id=' . $post_data['post_deleted_user']
					. (empty($post_data['post_deleted_from']) ? '' : ' AND topic_id=' . $post_data['post_deleted_from'])
			;
			$result = $db->sql_query($sql);
			$del_info = $db->sql_fetchrow($result);
			$db->sql_freeresult($result);
