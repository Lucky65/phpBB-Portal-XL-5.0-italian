...
if ($mode == 'whois' && $auth->acl_get('a_') && $session_id)
{
	...
	// Output the page
	page_header($config['sitename'] . ' : ' . $user->lang['WHO_IS_ONLINE']);
...
// Get user list + phpBB Gallery integration
...
while ($row = $db->sql_fetchrow($result))
{
	...
	switch ($on_page[1])
	{
		...
		// Toplist MOD
		case 'toplist':
			$location = $user->lang['VIEWING_TOPLIST'];
