...
* @file				dl_admin_files.php 5 2011/02/12 OXPUS
* @copyright		(c) 2006 oxpus (Karsten Ude) <webmaster@oxpus.de> http://www.oxpus.de
* @copyright mod	(c) hotschi / demolition fabi / oxpus
* @license			http://opensource.org/licenses/gpl-license.php GNU Public License 
* @based on			Module acp_profile.php 10216 from phpBB 3.0.7-PL1
*
*/
...
$create = request_var('create', '', true);
...
switch ($action)
{
	case 'delete':
		...
		if (confirm_box(true))
		{
			...
			$db->sql_query('DELETE FROM ' . DL_FIELDS_TABLE . " WHERE field_id = $field_id");
			$db->sql_query('DELETE FROM ' . DL_FIELDS_LANG_TABLE . " WHERE field_id = $field_id");
			$db->sql_query('DELETE FROM ' . DL_LANG_TABLE . " WHERE field_id = $field_id");
			...
			add_log('admin', 'DL_LOG_FIELD_REMOVED', $field_ident);
			trigger_error($user->lang['DL_FIELD_REMOVED'] . adm_back_link($this->u_action));
		}
		else
		{
	...
	case 'activate':
		...
		add_log('admin', 'DL_LOG_FIELD_ACTIVATE', $field_ident);
		trigger_error($user->lang['DL_FIELD_ACTIVATED'] . adm_back_link($this->u_action));
	...
	case 'deactivate':
		...
		add_log('admin', 'DL_LOG_FIELD_DEACT', $field_ident);
		trigger_error($user->lang['DL_FIELD_DEACTIVATED'] . adm_back_link($this->u_action));
	...
	case 'edit':
		...
		$req_next = request_var('next', '', true);
		$req_prev = request_var('prev', '', true);
		$req_save = request_var('save', '', true);
		...
		foreach ($exclude[2] as $key)
		{
			...
			else if ($field_type == FIELD_DATE && $key == 'field_default_value')
			{
				...
				{
					$reg_def_day = request_var('field_default_value_day', 0);
					if ($reg_def_day)
					{
			...
			else if ($field_type == FIELD_INT && $key == 'field_default_value')
			{
				...
				if (request_var('field_default_value', $field_row['field_default_value']) === '')
				{
					$var = '';
				}
			}
		...
		foreach ($exclude as $num => $key_ary)
		{
			...
			{
				$req_lang_opt = utf8_normalize_nfc(request_var($key, array(array('')), true));
				$reg_def_day = request_var('field_default_value_day', 0);
				...
				{
					$req_key = utf8_normalize_nfc(request_var($key, '', true));
					$req_ary = utf8_normalize_nfc(request_var($key, array(''), true));
		...
		$template->assign_vars(array(
			...
			'L_TITLE'			=> $user->lang['DL_FIELDS_STEP' . $step . '_TITLE_' . strtoupper($action)],
			'L_EXPLAIN'			=> $user->lang['DL_FIELDS_STEP' . $step . '_EXPLAIN'],
