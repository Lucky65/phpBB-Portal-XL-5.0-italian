...
* @file				dl_admin_categories.php 22 2011/02/12 OXPUS
...
{
	$error = true;
	$error_msg = sprintf($user->lang['DL_PATH_NOT_EXIST'], $path);
...
if($action == 'edit' || $action == 'add')
{
	...
	else
	{
		...
		$cat_parent			= '<select name="parent">';
		$cat_parent			.= '<option value="0">&nbsp;&raquo;&nbsp;'.$user->lang['DL_CAT_INDEX'].'</option>';
		$cat_parent			.= $dl_mod->dl_dropdown(0, 0, $cat_parent_id, 'auth_view', -1);
		$cat_parent			.= '</select>';
		$cat_remain_traffic	= $cat_traffic;
		$perm_cat_id		= $perms_copy_from;
		$perms_copy_from	= '<select name="perms_copy_from">';
		$perms_copy_from	.= '<option value="0">&nbsp;&raquo;&nbsp;'.$user->lang['DL_CAT_PARENT'].'</option>';
		$perms_copy_from	.= $dl_mod->dl_dropdown(0, 0, $perm_cat_id, 'auth_view', -1);
		$perms_copy_from	.= '</select>';
	}
...
else if($action == 'save_cat')  // Methode umstellen, damit Fehler angezeigt werden, aber auch die letzten Angaben wieder geladen werden. 
{
