...
* @file				dl_admin_permissions.php 8 2011/02/12 OXPUS
...
if (!sizeof($index))
{
	redirect(append_sid("{$phpbb_root_path}adm/index.$phpEx", "i=downloads&amp;mode=categories"));
}

$s_presel_cats		= request_var('cat_select', array(0));
$s_presel_groups	= request_var('group_select', array(0));
$view_perm			= request_var('view_perm', 0);
...
if ($view_perm > 1)
{
	...
	if (confirm_box(true))
	{
		if ($view_perm == 2)
		{
			...
			add_log('admin', 'DL_LOG_CAT_PERM_DROP', $cat_list);
		}
		else
		{
			$sql = 'DELETE FROM ' . DL_AUTH_TABLE;
			...
			add_log('admin', 'DL_LOG_CAT_PERM_ALL');
		}
	...
	{
		$confirm_text = ($view_perm == 2) ? sprintf($user->lang['DL_PERM_CATS_DROP_CONFIRM'], $cat_list) : $user->lang['DL_PERM_ALL_DROP_CONFIRM'];
	...
	if ($cancel)
	{
		$message = $user->lang['DL_PERM_DROP_ABORTED'] . '<br /><br />' . sprintf($user->lang['CLICK_RETURN_DOWNLOADADMIN'], "<a href=\"{$basic_link}\">", "</a>") . adm_back_link($this->u_action);
	}
	else
	{
		$message = $user->lang['DL_PERM_DROP'] . '<br /><br />' . sprintf($user->lang['CLICK_RETURN_DOWNLOADADMIN'], "<a href=\"{$basic_link}\">", "</a>") . adm_back_link($this->u_action);
	}
...
if ($view_perm == 1)
{
	if (isset($s_presel_cats[0]))
	{
		...
		{
			$auth_view	= ($row['auth_view']) ? '<strong>' . $user->lang['YES'] . '</strong>' : $user->lang['NO'];
			$auth_dl	= ($row['auth_dl']) ? '<strong>' . $user->lang['YES'] . '</strong>' : $user->lang['NO'];
			$auth_up	= ($row['auth_up']) ? '<strong>' . $user->lang['YES'] . '</strong>' : $user->lang['NO'];
			$auth_mod	= ($row['auth_mod']) ? '<strong>' . $user->lang['YES'] . '</strong>' : $user->lang['NO'];
		...
		{
			case 0:
				$l_auth_cread = $user->lang['DL_STAT_PERM_ALL'];
			break;
			case 1:
				$l_auth_cread = $user->lang['DL_STAT_PERM_USER'];
			break;
			case 2:
				$l_auth_cread = $user->lang['DL_STAT_PERM_MOD'];
			break;
			case 3:
				$l_auth_cread = $user->lang['DL_STAT_PERM_ADMIN'];
			break;
		}
		...
		{
			case 0:
				$l_auth_cpost = $user->lang['DL_STAT_PERM_ALL'];
			break;
			case 1:
				$l_auth_cpost = $user->lang['DL_STAT_PERM_USER'];
			break;
			case 2:
				$l_auth_cpost = $user->lang['DL_STAT_PERM_MOD'];
			break;
			case 3:
				$l_auth_cpost = $user->lang['DL_STAT_PERM_ADMIN'];
			break;
		}
	
		switch ($index[$cat_id]['auth_view'])
		{
			case 1:
				$l_auth_view = $user->lang['DL_PERM_ALL'];
			break;
			case 2:
				$l_auth_view = $user->lang['DL_PERM_REG'];
			break;
			default:
				$l_auth_view = $user->lang['DL_PERM_GRG'];
		}
	
		switch ($index[$cat_id]['auth_dl'])
		{
			case 1:
				$l_auth_dl = $user->lang['DL_PERM_ALL'];
			break;
			case 2:
				$l_auth_dl = $user->lang['DL_PERM_REG'];
			break;
			default:
				$l_auth_dl = $user->lang['DL_PERM_GRG'];
		}
	
		switch ($index[$cat_id]['auth_up'])
		{
			case 1:
				$l_auth_up = $user->lang['DL_PERM_ALL'];
			break;
			case 2:
				$l_auth_up = $user->lang['DL_PERM_REG'];
			break;
			default:
				$l_auth_up = $user->lang['DL_PERM_GRG'];
		}
	
		switch ($index[$cat_id]['auth_mod'])
		{
			case 1:
				$l_auth_mod = $user->lang['DL_PERM_ALL'];
			break;
			case 2:
				$l_auth_mod = $user->lang['DL_PERM_REG'];
			break;
			default:
				$l_auth_mod = $user->lang['DL_PERM_GRG'];
		}
...
if($action == 'save_perm')
{
	...
	$auth_cread	= request_var('auth_cread', 3);
	$auth_cpost	= request_var('auth_cpost', 3);

	switch($auth_view)
	{
		case 1:
			$log_auth_view = $user->lang['DL_PERM_ALL'];
		break;
		case 2:
			$log_auth_view = $user->lang['DL_PERM_REG'];
		break;
		default:
			$log_auth_view = $user->lang['DL_PERM_GRG'];
		break;
	}
	...
	{
		case 1:
			$log_auth_dl = $user->lang['DL_PERM_ALL'];
		break;
		case 2:
			$log_auth_dl = $user->lang['DL_PERM_REG'];
		break;
		default:
			$log_auth_dl = $user->lang['DL_PERM_GRG'];
		break;
	}
	...
	{
		case 1:
			$log_auth_up = $user->lang['DL_PERM_ALL'];
		break;
		case 2:
			$log_auth_up = $user->lang['DL_PERM_REG'];
		break;
		default:
			$log_auth_up = $user->lang['DL_PERM_GRG'];
		break;
	}
	...
	{
		case 1:
			$log_auth_mod = $user->lang['DL_PERM_ALL'];
		break;
		case 2:
			$log_auth_mod = $user->lang['DL_PERM_REG'];
		break;
		default:
			$log_auth_mod = $user->lang['DL_PERM_GRG'];
		break;
	}
	...
	{
		case 1:
			$log_auth_cread = $user->lang['DL_STAT_PERM_USER'];
		break;
		case 2:
			$log_auth_cread = $user->lang['DL_STAT_PERM_MOD'];
		break;
		case 3:
			$log_auth_cread = $user->lang['DL_STAT_PERM_ADMIN'];
		break;
		default:
			$log_auth_cread = $user->lang['DL_STAT_PERM_ALL'];
		break;
	}
	...
	{
		case 1:
			$log_auth_cpost = $user->lang['DL_STAT_PERM_USER'];
		break;
		case 2:
			$log_auth_cpost = $user->lang['DL_STAT_PERM_MOD'];
		break;
		case 3:
			$log_auth_cpost = $user->lang['DL_STAT_PERM_ADMIN'];
		break;
		default:
			$log_auth_cpost = $user->lang['DL_STAT_PERM_ALL'];
		break;
	}
	...
	if (isset($s_presel_groups[0]) && $s_presel_groups[0] == -1)
	{
		for ($i = 0; $i < sizeof($s_presel_cats); $i++)
		{
			...
			$sql = 'UPDATE ' . DL_CAT_TABLE . ' SET ' . $db->sql_build_array('UPDATE', array(
				...
				'auth_cpost'	=> $auth_cpost)) . ' WHERE id = ' . $s_presel_cats[$i];
			$db->sql_query($sql);

			add_log('admin', 'DL_LOG_CAT_PERM_ALL', $cat_name, $log_auth_view, $log_auth_dl, $log_auth_up, $log_auth_mod, $log_auth_cread, $log_auth_cpost);
		}
	}
	else
	{
		for ($i = 0; $i < sizeof($s_presel_cats); $i++)
		{
			...
			{
				$sql = 'DELETE FROM ' . DL_AUTH_TABLE . '
					WHERE cat_id = ' . $s_presel_cats[$i] . '
						AND group_id = ' . $s_presel_groups[$j];
				$db->sql_query($sql);
				...
				$sql = 'SELECT group_type, group_name FROM ' . GROUPS_TABLE . '
					WHERE group_id = ' . $s_presel_groups[$j];
				$result = $db->sql_query($sql);
				$row = $db->sql_fetchrow($result);
				$db->sql_freeresult($result);
				$group_name = ($row['group_type'] == GROUP_SPECIAL) ? $user->lang['G_' . $row['group_name']] : $row['group_name'];

				add_log('admin', 'DL_LOG_CAT_PERM_GRP', $cat_name, $group_name, $log_auth_view, $log_auth_dl, $log_auth_up, $log_auth_mod);
			}
		}
	}
...
if (sizeof($s_presel_cats))
{
	...
	if (!$view_perm)
	{
		...
		if ($total_groups)
		{
			...
			$s_group_select .= '<optgroup label="' . $user->lang['DL_PERMISSIONS_ALL'] . '">';
			$s_group_select .= '<option value="-1">' . $user->lang['DL_ALL'] . '</option>';
			$s_group_select .= '</optgroup>';
			$s_group_select .= '<optgroup label="' . $user->lang['USERGROUPS'] . '">';
		...
		if (sizeof($s_presel_groups))
		{
			...
			if ($s_presel_groups[0] == -1)
			{
				...
				$s_auth_all = '<option value="-1">' . $user->lang['SELECT_OPTION'] . '</option>';
				$s_auth_all .= '<option value="1">'.$user->lang['DL_PERM_ALL'].'</option>';
				$s_auth_all .= '<option value="2">'.$user->lang['DL_PERM_REG'].'</option>';
				$s_auth_all .= '<option value="0">'.$user->lang['DL_PERM_GRG'].'</option>';
				$s_auth_all .= '</select>';
				...
				$s_auth_all = '<option value="-1">' . $user->lang['SELECT_OPTION'] . '</option>';
				$s_auth_all .= '<option value="0">' . $user->lang['DL_STAT_PERM_ALL'] . '</option>';
				$s_auth_all .= '<option value="1">' . $user->lang['DL_STAT_PERM_USER'] . '</option>';
				$s_auth_all .= '<option value="2">' . $user->lang['DL_STAT_PERM_MOD'] . '</option>';
				$s_auth_all .= '<option value="3">' . $user->lang['DL_STAT_PERM_ADMIN'] . '</option>';
				$s_auth_all .= '</select>';
				...
				$template->assign_vars(array(
					'L_AUTH_EXPL'	=> (sizeof($s_presel_cats) == 1) ? $user->lang['DL_AUTH_SINGLE_EXPLAIN'] : $user->lang['DL_AUTH_MULTI_EXPLAIN'],
					'L_OPTIONS'		=> $user->lang['SELECT_OPTION'],
...
$s_cat_select .= $dl_mod->dl_cat_select(0, 0, $s_presel_cats);
$s_cat_select .= '</select>';
...
$template->set_filenames(array(
	'permissions' => 'dl_mod/dl_permissions_body.html')
);
