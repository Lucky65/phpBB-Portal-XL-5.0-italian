...
* @file				dl_admin_toolbox.php 10 2011/02/12 OXPUS
...
if (isset($index) && sizeof($index))
{
	...
	{
		$read_files = $dl_mod->read_dl_files($phpbb_root_path . $config['dl_download_dir'], '', $unas_files);
...
if ($action == 'browse' || $action == '' || $action == 'unassigned')
{
	if ($action != 'unassigned')
	{
		...
		while (false !== ($file=@readdir($dh)))
		{
			if (substr($file,0,1)!=".")
			{
				...
				{
					$files[] = '<a href="' . $phpbb_root_path . $config['dl_download_dir'] . $path . '/' . $file.'">' . $file . '</a>';
					$filen[] = $file;
	...
	if ($files)
	{
		...
		if ($action != 'unassigned')
		{
			...
			$s_file_action .= $dl_mod->read_dl_dirs($phpbb_root_path . $config['dl_download_dir']);
		}
		else
		{
