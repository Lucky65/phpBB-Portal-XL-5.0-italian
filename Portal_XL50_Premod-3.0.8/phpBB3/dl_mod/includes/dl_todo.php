...
* @file				dl_toto.php 5 2011/02/10 OXPUS
...
if ($submit && !$cancel)
{
	...
	if ($df_id)
	{
		...
		$sql = 'UPDATE ' . DOWNLOADS_TABLE . ' SET ' . $db->sql_build_array('UPDATE', array(
			...
			'todo_flags'	=> $todo_flags)) . " WHERE id = $df_id
...
{
	$dl_file = array();
	$dl_file = $dl_mod->all_files(0, '', 'ASC', '', $df_id, 0, 'description, desc_uid, desc_flags, todo, todo_uid, todo_flags');
...
$template->assign_vars(array(
	'ADD_TODO'			=> ($edit) ? $user->lang['DL_EDIT_FILE'] : $user->lang['NEW_POST'],
	'TODO_TEXT'			=> $todo,
