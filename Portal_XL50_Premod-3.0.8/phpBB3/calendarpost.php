...
{
	if( $auth->acl_get('u_calendar_create_recurring_events') )
	{
	...
	case 'post':
		...
		if ( $auth->acl_gets('u_calendar_create_public_events', 'u_calendar_create_group_events', 'u_calendar_create_private_events') )
		{
			...
			if( $submit )
			{
				...
				{
					case 2:
						if ( $auth->acl_get('u_calendar_create_public_events') )
						{
							$is_authed = true;
						}
					break;
					...
					case 1:
						if ( $auth->acl_get('u_calendar_create_group_events') )
						{
							$is_authed = true;
						}
					break;
					...
					case 0:
					default:
						if ( $auth->acl_get('u_calendar_create_private_events') )
						{
							$is_authed = true;
						}
					break;
...
---------------------------------------------*/
$s_track_rsvps = false;
if( $auth->acl_get('u_calendar_track_rsvps'))
{
	$s_track_rsvps = true;
}
$s_allow_guests = false;
if( $auth->acl_get('u_calendar_allow_guests'))
{
	$s_allow_guests = true;
}
...
if ($submit || $preview)
{
	...
	if (!sizeof($error) && $submit)
	{
		if ($submit)
		{
			...
			---------------------------------------------*/
			$main_calendar_url = append_sid("{$phpbb_root_path}calendar.$phpEx", "calM=".$date['month_no']."&amp;calY=".$date['year']);
...
// Output page ...
page_header($config['sitename'] . ' : ' . $page_title);
