...
if ($config['load_online'] && $config['load_online_time'] && $display_online_list)
{
	...
	if (!$config['load_online_guests'])
	{
		...
		$sql = 'SELECT u.username, u.username_clean, u.user_id, u.user_type, u.user_allow_viewonline, u.user_colour, user_country_flag, s.session_ip, s.session_viewonline
...
$lastvisits = $number_of_lastvisits_to_display;
	...
	$sql = 'SELECT user_id, username, user_colour, user_lastvisit, user_country_flag
		FROM ' . USERS_TABLE . '
	...
	while ($row = $db->sql_fetchrow($result) AND $lastvisits > 0)
	{
	  ...
	  $lastvisit_list .= (($lastvisit_list != '') ? '<br>' : '') . $user->format_date($row['user_lastvisit'], 'H:i') . ' ' . '<strong><a' . $user_colour . ' href="' . append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=viewprofile&amp;u=' . $row['user_id']) . '">' . $row['username'] . '</a></strong> ' . $flag_img . '';
	  $lastvisits --;
}
$db->sql_freeresult($result);
