...
if (sizeof($topic_list))
{
	...
	{
		$posttext[$row1['topic_id']] = $row1['post_text'];
	}
...
//		$s_type_switch_test = ($row['topic_type'] == POST_ANNOUNCE || $row['topic_type'] == POST_GLOBAL) ? 1 : 0;
		...
		// Ajax Userinfo MOD start 
		$last_post_author = '<a href="' . $phpbb_root_path . 'memberlist.php?mode=viewprofile&amp;u=' . $row['topic_last_poster_id'] . '"><b style="color:#' . $row['topic_last_poster_colour'] . '" onmouseover="show_popup(' .$row['topic_last_poster_id']. ')" onmouseout="close_popup()">' . $row['topic_last_poster_name'] . '</b></a>';
		// Ajax Userinfo MOD end 
...
//			'LAST_POST_AUTHOR_FULL'		=> get_username_string('full', $row['topic_last_poster_id'], $row['topic_last_poster_name'], $row['topic_last_poster_colour']),
			...
			// Ajax Userinfo MOD start 
			'LAST_POST_AUTHOR_FULL'		=> $last_post_author,
			// Ajax Userinfo MOD end 
