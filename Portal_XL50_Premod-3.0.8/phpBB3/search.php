...
*/
	...
	if ($mode='egosearch' && !empty($author_id_ary)) 	{
	  $template->assign_block_vars('navlinks', array(
		'FORUM_NAME' =>  ($author_id == $user->data['user_id'] ? $user->lang['SEARCH_SELF'] : $user->lang['POSTS']),
		'U_VIEW_FORUM' => (isset($u_search) ? $u_search : append_sid("{$phpbb_root_path}search.$phpEx"))
	  ));
	}   
	else {
	  $template->assign_block_vars('navlinks', array(
		'FORUM_NAME' => (($search->search_query) ? $search->search_query : $l_search_title),
		'U_VIEW_FORUM' => (isset($u_search) ? $u_search : append_sid("{$phpbb_root_path}search.$phpEx"))
	  ));
	}	
