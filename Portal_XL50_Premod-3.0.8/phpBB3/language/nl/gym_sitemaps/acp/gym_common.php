...
$lang = array_merge($lang, array(
	...
	'GYM_CLEAR_CACHE_EXPLAIN' => 'You can here clear the cached files for the %1$s module. These cached files contains the data used to build the %1$s output.<br/>It can be useful if you want to force the cache update.',
	'GYM_CLEAR_ACP_CACHE' => 'Clear %1$s ACP',
	'GYM_CLEAR_ACP_CACHE_EXPLAIN' => 'You can choose to clear the %1$s ACP cached setup instead. These cached files contains the data used to build the %1$s ACP.<br/>It can be useful to activate new options that may have been added to the modules of this output type.',
	...
	'GYM_CACHE_MAX_AGE_EXPLAIN' => 'Maximum amount of hours a cached file will be used before it will be updated. Each cached file will be updated everytime someone will browse it after this duration was exeeded when auto regen is on. If not, the cache will only be updated upon demand in ACP.',
	'GYM_CACHE_AUTO_REGEN' => 'Cache auto update',
	'GYM_CACHE_AUTO_REGEN_EXPLAIN' => 'If you activate the cache auto update, outputed lists will be updated once the cache will have expired, if not, you will have to manually clear your cached files in the maintenance link above to see new URLs in your lists.',
	...
	'GYM_MODRTYPE_EXPLAIN' => 'These options are overridden by the use of the phpBB SEO mod rewrite (auto detection ).<br/>Four levels of url rewriting can be set up here: None, Simple, Mixed and Advanced :<br/><ul><li><b>None :</b> No URL rewriting;<br></li><li><b>Simple :</b>Static URL rewriting for all links, no title injection;<br></li><li><b>Mixed :</b> Forum and category titles are injected in URLs, but topic titles remain staticly rewritten;<br></li><li><b>Advanced :</b> All titles are injected in URLs;</li></ul>',
	...
	'GYM_GZIP_EXT' => 'Gunzip suffixe',
	'GYM_GZIP_EXT_EXPLAIN' => 'You can here chose to use or not the .gz suffixe in the module URLs. This only applies when gunzip and URL rewriting are activated.',
	...
	'GYM_LIMIT' => 'Limits',
	'GYM_LIMIT_EXPLAIN' => 'Here you can set the limit to apply when building the output : number of url outputed, SQL cycling (amout of item queried at once) and age of items listed.<br/>Remember that these settings may be overridden depending on your override settings.',
	...
	'GYM_TIME_LIMIT_EXPLAIN' => 'Limit in days. The maximum age of the items taken into account when building the output lists. Can be very useful to lower the server load on large data bases. Enter 0 for no limit',
	...
	'GYM_SORT_TYPE_EXPLAIN' => 'All outputed links are sorted by default by last activity (Descending). <br /> You can set this to Ascending for example if you wish to make it easier for Search engines to find links to old content.<br/>Remember that these settings may be overridden depending on your override settings.',
