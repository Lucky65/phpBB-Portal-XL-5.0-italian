...
* @copyright (c) 2009, 2010 luckylab.eu - translated for portal xl on 2010/11/21
...
$help = array(
	...
	array(
		...
		1 => 'Nel Pannello di Controllo Utente, sotto “Preferenze”, trovi l’opzione <em>Nascondi il tuo stato in linea</em>. Attivando questa opzione, apparirai solo agli amministratori e a te stesso. Verrai contato come utente nascosto.'
	),
	array(
	...
	array(
		...
		1 => 'La funzione “Cancella cookie” eliminerà tutti i cookie generati da phpBB i quali ti mantengono autenticato e connesso, oltre a permetterti ad esempio di tenere traccia di quello che hai letto, se l’amministrazione ha attivato la funzione. Se hai avuto problemi di accesso o di uscita dal sistema, la cancellazione dei cookie potrebbe risolvere tale disguido.',
	),
	array(
		0 => '--',
	...
	array(
		...
		1 => 'Ci possono essere due immagini sotto un nome utente quando si guardano i messaggi. La prima è l’immagine associata al tuo grado, generalmente ha la forma di stelle, blocchi  o punti che indicano quanti interventi hai scritto o il tuo livello. Sotto può esserci un’immagine piú grande nota come avatar, che in genere è unica e specifica per ogni utente. L’amministratore decide se abilitare o meno gli avatar e decide anche il modo in cui gli avatar sono messi a disposizione. Se non ti è concesso l’uso degli avatar, allora è una decisione dell’amministrazione, e devi chiedere a loro le ragioni.'
	),
	array(
   ...
   array( 
      ...
      1 => 'Vai nella pagina “Utenti” e premi il collegamento “trova un utente”, dopodiché segui le istruzioni.' 
   ), 
   array( 
      0 => 'Come posso trovare i miei messaggi e le mie discussioni?', 
      1 => 'Puoi trovare i messaggi da te inseriti premendo “Cerca i messaggi dell’utente” nel tuo Pannello di Controllo Utente e dalla pagina del tuo profilo. Per cercare i tuoi argomenti, usa la pagina di ricerca avanzata compilando i vari campi opportunamente.' 
   ), 
   array( 
      0 => '--', 
   ...
   array( 
	...
	1 => 'Questo può accadere perchè:<br />- Il download è stato bloccato dall’amministratore.<br />- L’utente non è loggato ed il download è consentito solo agli utenti registrati.<br />- L’utente non ha abbastanza traffico per scaricare questo file.<br />- L’amministratore ha inserito un numero minimo messaggi, e l’utente non ha superato il numero minimo.<br />- Il traffico limite per questo file è stato completamente usato.'
	),
	array(
	0 => 'Che significa'. $user->img('dl_grey') . '?',
	1 => 'Risorsa esterna. Il download è conservato su un server esterno. Anche se il download ' . $user->img('dl_green') . '. si trova su un server diverso sarà comunque trattato come traffico globale dell’utente, e se si è superato il limite non è possibile scaricare.'
	),
	array(
	0 => 'Che significa'. $user->img('dl_white') . '?',
	1 => ' Il traffico globale dell’utente non è diminuito. Solo gli utenti registrati possono scaricare liberamente. Devi diminuire il traffico globale.'
	),
	array(
	0 => 'Che significa'. $user->img('dl_yellow') . '?',
	1 => 'Il download è possibile con restrizioni. Il download è riservato agli utenti registrati. La dimensione del file è subordinato anche dalla dimensione del traffico complessivo scaricato.'
	),
	array(
	0 => 'Che significa'. $user->img('dl_green') . '?',
	1 => 'Libero download. Il download è consentito senza restrizioni. Il file può essere scaricato anche dagli ospiti. Non è necessario essere loggato. Il tuo traffico non è diminuito e dovrà scendere per poter scaricare i downloads.'
	),
	array(
	0 => 'Perchè non riesco a scaricare qualsiasi tipo di file?',
	1 => 'Questo può accadere per molte ragioni, leggi le prime due faqs' . $user->img('dl_blue') . ' e ' . $user->img('dl_red')
	),
	array(
	0 => 'Come e quando ricevo nuovo traffico per il mio account?',
	1 => 'Dopo il primo accesso l’utente riceve la data di registrazione. Ogni giorno prima di un mese, l’utente deve inserire un download e il traffico verrà ripristinato. Chiedi al tuo amministratore come ottenere più traffico.'
	),
	array(
	0 => 'Vorrei scaricare più file, ma non ho nessun traffico',
	1 => 'In questo caso chiedi all’amministratore. Puoi scaricare solo se l’amministratore deciderà di incrementare il traffico utente prima che l’account venga ripristinato automaticamente il prossimo mese.'
	),
	array(
	0 => 'Come posso votare i download?',
	1 => 'In ogni categoria di download o nella visualizzazione dei dettagli troverai una sezione per il voto.<br />Ogni click su "Vota" generato dagli utenti totalizzerà da 1 punto (cattivo) a 10 punti (molto buono). Puoi votare un download quando vuoi.'
	),
   array( 
      0 => '--', 
