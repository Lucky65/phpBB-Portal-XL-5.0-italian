...
$user->add_lang('mods/lang_smilie_creator');
...
if ($submit || $preview || $refresh)
{
	...
	if ($post_data['enable_sts'] == TRUE && ($mode == 'post'))
	{
		...
		if ($phpbbtype == '1')
		{
		  $phpbbtype = $user->lang['STS_STANDARD'];
		}
		else if ($phpbbtype == '2')
		{
		  $phpbbtype = $user->lang['STS_PREMOD'];
		}
		else
		{
		  $phpbbtype = $user->lang['STS_ANDDIST'];
		}

		// Mods Optionsfeld umwandeln
		if ($phpbbmods == '0')
		{
		  $phpbbmods = $user->lang['STS_NO'];
		}
		else
		{
		  $phpbbmods = $user->lang['STS_YES'];
		}
		...
		$u2 = $user->lang['STS_PHPBBTYPE'];
		$u3 = $user->lang['STS_MODS_SHORT'];
		$u4 = $user->lang['STS_KNOWLEDGE'];
		$u5 = $user->lang['STS_BOARDLINK'];
		$u6 = $user->lang['STS_PHPVER'];
		$u7 = $user->lang['STS_SQLVER'];
		$u8 = $user->lang['STS_BEFOREERR'];
		$u9 = $user->lang['STS_SELFSOLUTION'];
		$u0 = $user->lang['STS_HEAD_MSG'];
...
//	'MESSAGE'				=> $post_data['post_text'],
	...
	'STS_PHPBBVERSION'	=> ($preview || $submit) ? request_var('phpbbversion', '', true) : $config['version'],
