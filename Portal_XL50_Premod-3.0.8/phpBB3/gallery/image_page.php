...
$sort_order = request_var('sort_order', $gallery_config['sort_order']);
$comments_per_page = $config['posts_per_page'];
$total_comments = (int) $image_data['image_comments'];
$phpbb_seo->prepare_url('pic', $image_data['image_name'], $image_id, $phpbb_seo->seo_url['album'][$album_id]);
// Make sure $start is set to the last page if it exceeds the amount
if ($start < 0 || $start > ($total_comments - 1) ) {
	$start = ($start < 0) ? 0 : floor(($total_comments - 1) / $comments_per_page) * $comments_per_page;
}
$phpbb_seo->seo_opt['zero_dupe']['start'] = $phpbb_seo->seo_chk_start( $start, $comments_per_page );
// Prevent unnecessary redirect upon POST submit
$keep_options = (boolean) !isset($_POST['submit']);
$phpbb_seo->seo_opt['zero_dupe']['redir_def'] = array(
	'album_id' => array('val' => $album_data['album_id'], 'keep' => true, 'force' => true),
	'image_id' => array('val' => $image_id, 'keep' => true, 'force' => true),
	'sort_order' => array('val' => $sort_order, 'keep' => $keep_options),
...
$template->assign_vars(array(
	...
	'IMAGE_BBCODE' => '[img]' . ($_img_url = $phpbb_seo->drop_sid(append_sid((empty($phpbb_seo->seo_opt['url_rewrite']) ? generate_board_url(false) . '/' : $phpbb_root_path ) . "{$gallery_root_path}image.$phpEx?album_id=$album_id&amp;image_id=$image_id"))) . '[/img]',
	'IMAGE_IMGURL_BBCODE' => ($gallery_config['view_image_url']) ? '[url=' . $_img_url . '][img]' . $phpbb_seo->drop_sid(append_sid((empty($phpbb_seo->seo_opt['url_rewrite']) ? generate_board_url(false) . '/' : $phpbb_root_path ) . "{$gallery_root_path}image.$phpEx?album_id=$album_id&amp;image_id=$image_id&amp;mode=thumbnail")) . '[/img][/url]' : '',
	'IMAGE_URL' => ($gallery_config['view_image_url']) ? $_img_url : '',
