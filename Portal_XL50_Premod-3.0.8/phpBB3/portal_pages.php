...
$user->session_begin();
$auth->acl($user->data);
$user->setup(array('viewtopic', 'mods/portal_xl'));
...
*/
		...
		$template->assign_block_vars('pagepage', array(
	      ...
	      'U_PRINT_PAGES'		=> append_sid("{$phpbb_root_path}portal_pages.$phpEx", "page=" . $pagepage[$i]['page_id'] . "&amp;view=print"),
		  //'U_PRINT_PAGES'		=> ($auth->acl_get('f_print', $pagepage[$i]['page_id'])) ? append_sid("{$phpbb_root_path}portal_pages.$phpEx") . '&amp;view=print' : '',
		));
		...
		$template->set_filenames(array(
			'body' => ($view == 'print') ? 'portal/portal_pages_page_body_print.html' : 'portal/portal_pages_page_body.html',
...
else
{
	...
	//-- mod: Prime Links -------------------------------------------------------//
		...
		$page_content = prime_links($page_content);
	//-- end: Prime Links -------------------------------------------------------//
