...
function prime_quick_reply_onload()
{
	if (is_ie && typeof(baseHeight) != 'number')
	{
		window.onload = prime_quick_reply_fix_position; // override initInsertions()
	}
}
prime_quick_reply_onload();
...
{
	var v, n, area_obj = document.getElementById(active_obj.id.replace('toggle_', ''));
	if (typeof(active_obj.checked) != 'undefined')
	{
		n = area_obj.id.replace('quick_reply_', '');
		v = active_obj.checked;
	}
	else
	{
		n = (area_obj.id == 'postform') ? 'form' : 'options';
		v = (area_obj.style.display == 'none');
		eval('active_obj.innerHTML = LA_' + (v ? 'HIDE' : 'SHOW') + '_' + (n == 'form' ? 'QUICK_REPLY' : 'OPTIONS') + ';');
	}
	area_obj.style.display = v ? '' : 'none';
	if (n && QUICK_REPLY_COOKIE)
	{
	   	document.cookie = QUICK_REPLY_COOKIE + '_' + n + '=' + (v ? 1 : 0) + '; expires=' + (new Date((new Date()).getTime()+30*86400000)).toGMTString();
	}
	if (n == 'form')
	{
	...
	if (area_obj.id == 'quick_reply_bbcodes')
	{
		area_obj = document.getElementById('quick_reply_palette');
...
function prime_quick_reply_check()
{
	...
	if (typeof(prime_post_ids) == 'object')
	{
