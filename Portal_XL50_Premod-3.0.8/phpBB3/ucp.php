...
if ($mode == 'login' )
{
  ...
  'FORUM_NAME' => ($user->lang['LOGIN']),
  'U_VIEW_FORUM' => append_sid("{$phpbb_root_path}ucp.$phpEx", 'mode=login')
  ));
}
else if ($mode == 'register' )
{
  $template->assign_block_vars('navlinks', array(
  'FORUM_NAME' => ($user->lang['REGISTER']),
  'U_VIEW_FORUM' => append_sid("{$phpbb_root_path}ucp.$phpEx", 'mode=register')
  ));
}
...
$template->assign_block_vars('navlinks', array(
	'FORUM_NAME' => ($module->get_page_title())
));
