...
class install_update extends module
{
	...
	*/
	function check_server_requirements($mode, $sub)
	{
		...
		include($phpbb_root_path . 'install_gallery/outdated_files.' . $phpEx);
		...
		$url = (!in_array(false, $passed)) ? append_sid("{$phpbb_root_path}install_gallery/index.$phpEx", "mode=$mode&amp;sub=update_db") : append_sid("{$phpbb_root_path}install_gallery/index.$phpEx", "mode=$mode&amp;sub=requirements");
	...
	*/
	function update_db_schema($mode, $sub)
	{
		...
		$template->assign_vars(array(
			...
			'L_SUBMIT'	=> $user->lang['NEXT_STEP'],
			'S_HIDDEN'	=> '',
			'U_ACTION'	=> append_sid("{$phpbb_root_path}install_gallery/index.$phpEx", "mode=$mode&amp;sub=update_db&amp;step=2"),
		));
	}
	...
	function update_db_data($mode, $sub)
	{
		...
		switch ($gallery_config['phpbb_gallery_version'])
		{
			...
			case '0.4.1':
				...
				$next_update_url = append_sid("{$phpbb_root_path}install_gallery/index.$phpEx", "mode=$mode&amp;sub=update_db&amp;step=3");
			break;
			...
			case '1.0.5-RC1':
			case '1.0.5':
				$next_update_url = append_sid("{$phpbb_root_path}install_gallery/index.$phpEx", "mode=$mode&amp;sub=update_db&amp;step=4");
			break;
		}

		$next_update_url = (!$next_update_url) ? append_sid("{$phpbb_root_path}install_gallery/index.$phpEx", "mode=$mode&amp;sub=update_db&amp;step=4") : $next_update_url;
...
/*			case '0.1.2':
		...
		{
			$next_update_url = append_sid("{$phpbb_root_path}install_gallery/index.$phpEx", "mode=$mode&amp;sub=final");
		}
