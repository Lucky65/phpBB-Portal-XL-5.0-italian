...
class install_uninstall extends module
{
	...
	*/
	function check_requirements($mode, $sub)
	{
		...
		$url = (!in_array(false, $passed)) ? append_sid("{$phpbb_root_path}install_gallery/index.$phpEx", "mode=$mode&amp;sub=delete_tables") : append_sid("{$phpbb_root_path}install_gallery/index.$phpEx", "mode=$mode&amp;sub=requirements");
	...
	*/
	function delete_tables($mode, $sub)
	{
		...
		$template->assign_vars(array(
			...
			'L_SUBMIT'	=> $user->lang['NEXT_STEP'],
			'S_HIDDEN'	=> '',
			'U_ACTION'	=> append_sid("{$phpbb_root_path}install_gallery/index.$phpEx", "mode=$mode&amp;sub=final"),
		));
	}
}
