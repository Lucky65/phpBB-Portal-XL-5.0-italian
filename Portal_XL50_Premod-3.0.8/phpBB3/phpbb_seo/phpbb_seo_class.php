...
class phpbb_seo extends setup_phpbb_seo {
	...
	*/
	function phpbb_seo() {
		...
		// stats
		global $starttime;
		$this->starttime = ( $starttime > 0 ) ? $starttime : $this->microtime_float();
		$this->seo_stats['format_url_time'] = $this->seo_stats['url_rewrite_time'] = $this->seo_stats['url_cached_time'] = 0;
		$this->seo_stats['format_url_num'] = $this->seo_stats['url_rewrite_num'] = $this->seo_stats['url_cached_num'] = 0;
		// stats
	...
	*/
	function url_rewrite($url, $params = false, $is_amp = true, $session_id = false) {
		...
		// stats : start here because the above code replaces the one from append_sid()
		// which is in all cases used (with and without rewriting)
		$startime = $this->microtime_float();
		...
		if (isset($this->seo_cache[$url])) {
			...
			// stats
			$this->seo_stats['url_cached_time'] = $this->seo_stats['url_cached_time'] + ($this->microtime_float() - $startime);
			$this->seo_stats['url_cached_num']++;
		...
		if ( !empty($this->rewrite_method[$this->path][$this->filename]) ) {
			...
			// stats
			$this->seo_stats['url_rewrite_time'] = $this->seo_stats['url_rewrite_time'] + ($this->microtime_float() - $startime);
			$this->seo_stats['url_rewrite_num']++;
	...
	*/
	function seo_end($return = false) {
		...
		if ($this->seo_opt['copyrights']['img']) {
			$output = '<span style="margin-top: 5px; text-align: middle;"> <a href="http://www.phpbb-seo.com/" title="' . $this->seo_opt['copyrights']['title'] . '"><img src="' . $this->seo_path['phpbb_url'] . 'images/phpbb-seo.png" alt="' . $this->seo_opt['copyrights']['txt'] . '"/></a></span>';
		} else {
			$output = '<span style="margin-top: 5px; text-align: middle;"> <a href="http://www.phpbb-seo.com/" title="' . $this->seo_opt['copyrights']['title'] . '">' . $this->seo_opt['copyrights']['txt'] . '</a></span>';
		}
		if ($return) {
			return $output;
		} else {
